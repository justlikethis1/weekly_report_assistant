...
class ReportValidator:
    ...
    def validate(self, report_content: str) -> Dict[str, Any]:
        """
        验证报告内容的质量和正确性
        ...
        try:
            ...
            # 检查核心主题
            if "黄金价格" not in report_content:
                validation_result["errors"].append("缺少核心主题：黄金价格")
                validation_result["is_valid"] = False
            if "2026" not in report_content:
                validation_result["errors"].append("缺少时间范围：2026年")
                validation_result["is_valid"] = False
            
            logger.info(f"Report validation completed. Word count: {len(report_content)}, Coverage: {validation_result['coverage']}")
